...
transformers_logging.set_verbosity_error()   # ← temperature 경고 제거
...
SPEED          = 20
SPEED_J        = 30
GRIPPER_OFFSET = 30    # 물체 표면에서 추가로 내려갈 mm
APPROACH_DIST  = 80    # 물체 위 접근 높이 (mm)
LIFT_DIST      = 120   # 픽킹 후 들어올리기 (mm)

# ── 픽킹 위치 오프셋 보정 ─────────────────────────────
# 항상 오른쪽으로 3cm 치우치면 → 반대 방향으로 -30 입력
# 로봇 베이스 좌표계 기준 (mm)
# 어느 축이 "오른쪽"인지 모르면 X, Y 둘 다 ±30씩 시도
PICK_OFFSET_X =   0   # mm  (+: X+ 방향,  -: X- 방향)
PICK_OFFSET_Y =   0   # mm  (+: Y+ 방향,  -: Y- 방향)
...
# ★ Place 위치: 티칭펜던트로 위치 잡고 좌표 입력
# 예) PLACE_J = [10.0, -90.0, 80.0, -85.0, -90.0, 0.0]
PLACE_J = [86.3, -56.45, 37.47, -94.04, -76.52, -111.0]   # TCP: [104, -798, 469]

# 스캔 1 위치에서 TCP Z축으로만 이동 (XY 고정)
# 순서: 원위치 → +5cm → -5cm
SCAN_Z_OFFSETS_MM = [0, 50, -50]

# VLM Lock: 스레드 간 GPU 충돌 방지
vlm_lock = threading.Lock()


# ══════════════════════════════════════════════════════
# 자연어 타겟 파싱
# "컵 잡아줘" → "cup or mug" 처럼 영어 VLM 친화적으로 변환
# ══════════════════════════════════════════════════════
KO_TO_EN = {
    "컵": "cup or mug",
    "머그컵": "mug",
...
def parse_target(command: str) -> str:
    """
    자연어 명령에서 타겟 물체를 추출하고 VLM용 영어로 변환.
    예) "컵 잡아줘" → "cup or mug"
        "빨간 병 집어줘" → "red bottle"
        "box pick" → "box"
    """
    ...
    for ko, en in KO_TO_EN.items():
        if ko in cmd:
            # 수식어(색상 등) 앞에 붙어있는 경우 처리
            cmd = cmd.replace(ko, en)
            return cmd.strip()

    # 이미 영어거나 매핑 없는 경우 그대로 반환
...
def wait_done(timeout: float = 30.0):
    """
    로봇 이동 완료까지 블로킹.
    - 명령 직후 done=1이 즉시 반환되는 오작동 방지를 위해
      0.4초 대기 후 폴링 시작 (motion이 시작될 시간 확보)
    - done==1 확인 후 추가 0.1초 settle 대기
    """
    time.sleep(0.4)          # 모션 시작 대기
    start = time.time()
    while time.time() - start < timeout:
        ret  = robot.GetRobotMotionDone()
        done = ret[1] if isinstance(ret, (list, tuple)) else ret
        if done == 1:
            time.sleep(0.1)  # 정착 대기
            return
        time.sleep(0.05)
...
def get_stable_pose(samples: int = 5, interval: float = 0.05) -> list:
    """
    TCP 포즈를 여러 번 읽어 평균을 반환.
    로봇이 완전히 멈췄는지 확인하는 용도.
    """
    poses = []
...
Important: If multiple objects exist, return ONLY the one that best matches '{target}'.
Detect even if partially visible or at an angle."""
    ...
    # ── 케이스 3: JSON 파싱 실패 → 정규식으로 bbox/label 쌍 추출 ──
    ...
    if all_bboxes:
        ...
        return {
            ...
            "bbox":  [int(v) for v in chosen_bbox],
            "confidence": "high"
        }
...
def do_pick(target_xyz):
    """
    접근 → 그립 → 들어올리기 → Place 이동 → 내려놓기 → 그리퍼 열기 → 홈
    """
    tx, ty, tz = target_xyz
...
# 지목 시 X 방향 오프셋 (mm) — 양수면 로봇 X+ 방향
POINT_OFFSET_X = 50


def do_point(target_xyz):
    """
    물체 위치 + X방향 50mm 지점으로 이동 → 잠시 대기 → 홈 복귀
    "박스 어디 있어?" 같은 지목 동작
    """
    tx, ty, tz = target_xyz
    current_pose = get_stable_pose(samples=3, interval=0.05)
    rx, ry, rz   = current_pose[3], current_pose[4], current_pose[5]
    point_z      = tz + APPROACH_DIST   # 물체 위 접근 높이
...
task_running   = False
task_status    = "대기 중"
current_target = ""           # 현재 실행 중인 타겟 (GUI 표시용)
...
def scan_and_detect(target: str) -> tuple:
    """
    스캔 1 도착 직후 호출.
    XY 고정, Z만 [0, +5cm, -5cm] 순으로 이동하며 자동 감지.
    1차 감지 성공 시 물체 방향으로 200mm 직진 후 재감지.
    성공: (robot_xyz, avg_bbox, avg_depth)
    실패: (None, None, None)
    """
    ...
    print(f"\n  ❌ '{target}' 를 찾지 못했습니다. (원위치 / +5cm / -5cm 모두 시도)")
    return None, None, None


def _go_scan1_and_detect(target: str) -> tuple:
    """홈 → 경유점 → 스캔1 이동 후 scan_and_detect 호출."""
    global task_status
    task_status = "홈 이동 중"
    move_j(HOME_J, "홈 이동")
    move_j(WAY_J,  "경유점 이동")
...
# ══════════════════════════════════════════════════════
# 한글 폰트가 없으면: sudo apt install fonts-nanum
...
def put_kr(img_bgr: np.ndarray, text: str, xy: tuple,
           font: ImageFont.FreeTypeFont,
           color_rgb: tuple = (255, 255, 255)) -> np.ndarray:
    """
    OpenCV BGR numpy 배열에 한글 텍스트를 PIL로 그려서 반환.
    color_rgb: (R, G, B) 형식
    """
...
def draw_ui(canvas: np.ndarray) -> np.ndarray:
    """상단 UI 패널을 PIL로 그리기 (한글 완전 지원)"""
...
def draw_result(frame_bgr: np.ndarray) -> np.ndarray:
    """카메라 프레임에 감지 결과 오버레이 (bbox, 좌표 등)"""
    vis = frame_bgr.copy()
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            if key == 13:
                ...
                if not cmd:
                    continue
                if task_running:
                    print("⚠ 현재 작업 진행 중입니다. STOP 후 재시도하세요.")
                    continue

                task_running   = True
                task_status    = "시작 중"
                current_target = parse_target(cmd)   # GUI 즉시 반영
                threading.Thread(
        ...
        if action_queue:
            ...
            if action == "detect" and not task_running:
                cmd = input_text.strip() or \
                      (current_target if current_target else "")
                if not cmd:
                    print("⚠ 타겟을 먼저 입력하세요.")
                else:
                    task_running   = True
