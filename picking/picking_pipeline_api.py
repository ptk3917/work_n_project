"""
FR5 + D455 + AIXERA_LLM API 통합 픽킹 파이프라인
【AIXERA LLM API 버전】 — 로컬 VLM → AIXERA_LLM API 교체
...
【변경된 흐름】
  "컵 잡아줘" 입력 + 엔터
      ↓
  타겟 파싱 → 스캔 포인트 1 이동 → 1차 감지 (AIXERA_LLM API)
      ↓ 감지됨
...
API_BASE_URL = "http://121.137.1.107:21004/v1"
API_KEY      = "none"                    # 인증 불필요
API_MODEL    = "AIXERA_LLM"
...
WAY_J   = [10.0, -98.0, 100.0, -94.0, -84.0, -111.0]   # HOME과 동일 = 스킵
...
# ══════════════════════════════════════════════════════
print("=" * 55)
print(f"VLM: {API_MODEL} API ({API_BASE_URL})")
print(f"  → GPU VRAM 0 사용, 로컬 모델 없음")
...
try:
    ...
    if ret == 0 and error != 0:
        ...
        print(f"  ⚠ 로봇 상태: ret={ret}, 클리어 시도...")
        robot.ResetAllError()
        time.sleep(0.5)
except Exception as e:
    print(f"  ⚠ 에러 확인 실패: {e} — 계속 진행")
print("✅ FR5 연결 완료")
...
def _pil_to_base64(image: Image.Image) -> str:
    """PIL 이미지 → base64 PNG 문자열 (무손실, bbox 정밀도 향상)"""
    buf = io.BytesIO()
...
def extract_bbox(image: Image.Image, target: str) -> dict:
    """
    AIXERA_LLM API로 물체 감지.
    """
    w, h = image.size

    prompt = f"""You are a precise object detection system for a robotic arm.
Task: Find '{target}' in this {w}x{h} image and return its TIGHT bounding box.

CRITICAL RULES:
1. The bounding box must TIGHTLY fit the object — no extra margin, no padding.
   BAD:  bbox covers 50% of the image
   GOOD: bbox wraps the object edges closely
2. Coordinates are in PIXELS: x ranges 0-{w}, y ranges 0-{h}.
3. Format: [x_min, y_min, x_max, y_max] where (x_min,y_min)=top-left, (x_max,y_max)=bottom-right.
4. A typical object in this image occupies roughly 50-200 pixels in each dimension, NOT the entire image.

Return ONLY this JSON, nothing else:
{{"found": true, "label": "object name", "bbox": [x_min, y_min, x_max, y_max]}}

If not found:
{{"found": false, "label": "", "bbox": []}}

If multiple candidates exist, pick the single best match for '{target}'."""
    ...
    try:
        ...
        print(f"  [AIXERA_LLM] ({elapsed:.1f}s) {raw}")

    except Exception as e:
        print(f"  [AIXERA_LLM 에러] {e}")
        return {"found": False, "bbox": [], "confidence": "none", "label": "api_error"}

    # ── JSON 파싱 ──────────────────────────────────────
    raw_clean = re.sub(r"```json|```", "", raw).strip()

    def _validate_bbox(bbox, w, h):
        """bbox가 합리적인 크기인지 검증 (이미지의 50% 이상이면 거부)"""
        ...
        ratio = (bw * bh) / (w * h)
        if ratio > 0.5:
            print(f"  ⚠ bbox가 이미지의 {ratio*100:.0f}%를 차지 → 부정확, 거부")
            return False
        return True
    ...
    if dict_match:
        try:
            ...
            if bbox and len(bbox) == 4 and item.get("found", True):
                ...
                if _validate_bbox(bbox_int, w, h):
                    return {
                        "found": True,
    ...
    if all_bboxes:
        bb = [int(v) for v in all_bboxes[0]]
        if _validate_bbox(bb, w, h):
            return {
                "found": True,
                "label": target,
                "bbox":  bb,
...
WINDOW_NAME = "Picking Pipeline (AIXERA_LLM API)"
CAM_H   = 480
CAM_W   = 640
INFO_H  = 95
...
def draw_result(frame_bgr):
    ...
    if last_result.get("found") and len(last_result.get("bbox", [])) == 4:
        ...
        vis = put_kr(vis, "AIXERA_LLM 응답 대기 중...", (16, 446), FONT_LG, (0, 200, 255))
    return vis
