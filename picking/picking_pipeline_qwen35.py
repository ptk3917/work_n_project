"""
FR5 + D455 + Qwen3.5-4B 통합 픽킹 파이프라인
【Qwen3.5 버전】 — Qwen2.5-VL → Qwen3.5-4B 교체

Qwen3.5는 비전이 내장된 통합 모델 (별도 "-VL" 라인 없음)
- 모델:    Qwen/Qwen3.5-4B  (bf16 ~8GB → 15GB GPU 여유)
- 클래스:  Qwen3_5ForConditionalGeneration
- 프로세서: AutoProcessor (apply_chat_template 으로 이미지+텍스트 한번에)
...
실행: python picking_pipeline_qwen35.py

※ 필요 패키지:
   pip install "transformers>=4.50" torch qwen-vl-utils
"""
import sys
import os
import warnings
...
# ── CUDA 메모리 단편화 방지 (반드시 torch import 전에 설정) ──
...
SCAN1_J = [-10.26, -169.9, 145.79, -111.88, -76.53, -111.69]
...
# 1. 모델 로드  ★ Qwen3.5-4B ★
# ══════════════════════════════════════════════════════
# bf16 ~8GB → 15GB GPU에 전체 배치, CPU 오프로드 불필요
# Qwen2.5-VL-7B(~14GB)보다 작아서 여유 있음
# Qwen3.5는 통합 모델이라 별도 "-VL" 없이 이미지 처리 가능
#
...
print(f"✅ Qwen3.5-4B 로드 완료 | VRAM: {vram_used:.1f} / {gpu_total_gb:.1f} GB")
...
if ret == 0 and error != 0:
    ...
    ret, error = robot.GetRobotErrorCode()
    if error != 0:
        print(f"  ⚠ 에러 클리어 실패 (code={error}), 티칭펜던트 확인 필요")
    else:
        print(f"  ✅ 에러 클리어 완료")
elif ret != 0:
...
def extract_bbox(image: Image.Image, target: str) -> dict:
    """
    Qwen3.5-4B 통합 모델로 물체 감지.
    apply_chat_template(tokenize=True) 로 이미지+텍스트 한번에 처리.
    """
    w, h = image.size
...
Do not output any thinking or reasoning, only the JSON."""
    ...
    # ── Qwen3.5: apply_chat_template 에서 직접 tokenize ──
    try:
        # 방법 1: apply_chat_template(tokenize=True) — Qwen3.5 권장
        ...
        # 방법 2: fallback — Qwen2.5-VL 스타일 (호환성)
    ...
    del out, inputs, generated   # ← 즉시 메모리 해제
    torch.cuda.empty_cache()
    print(f"  [VLM 출력] {raw.strip()}")

    # ── JSON 파싱 (Qwen2.5-VL 버전과 동일) ──────────
...
WINDOW_NAME = "Picking Pipeline (Qwen3.5-4B)"
CAM_H   = 480
CAM_W   = 640
INFO_H  = 95
